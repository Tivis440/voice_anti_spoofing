...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Исследование влияния dropout на качество модели"
    )
    ...
    parser.add_argument(
        ...
        help="Список значений dropout через запятую",
    )
    parser.add_argument(
        "--out-prefix",
        type=str,
        default="dropout_study",
...
def save_plot(df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.plot(df["dropout"], df["val_accuracy"], marker="o", label="val_accuracy")
    ax.plot(df["dropout"], df["val_macro_f1"], marker="o", label="val_macro_f1")
    ax.plot(df["dropout"], df["test_accuracy"], marker="o", label="test_accuracy")
    ax.plot(df["dropout"], df["test_macro_f1"], marker="o", label="test_macro_f1")
    ax.set_xlabel("Dropout")
    ax.set_ylabel("Метрика")
    ax.set_title("Влияние dropout на качество модели")
...
def main() -> int:
    ...
    dropout_values = [float(v.strip()) for v in args.dropout_values.split(",") if v.strip()]
    results: list[dict[str, float | int]] = []

    for dropout in dropout_values:
        print(f"\n=== dropout={dropout} ===")
        metrics = train_single_run(x_train, y_train, x_val, y_val, x_test, y_test, cfg, dropout)
        metrics["dropout"] = dropout
        metrics["feature_count"] = len(feature_names)
        results.append(metrics)

    results_df = pd.DataFrame(results).sort_values(["dropout"]).reset_index(drop=True)
