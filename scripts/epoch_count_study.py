...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Исследование влияния количества эпох на качество модели"
    )
    ...
    parser.add_argument(
        ...
        help="Список значений epochs через запятую",
    )
    parser.add_argument(
        "--out-prefix",
        type=str,
        default="epoch_count_study",
...
def save_plot(df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.plot(df["epochs"], df["val_accuracy"], marker="o", label="val_accuracy")
    ax.plot(df["epochs"], df["val_macro_f1"], marker="o", label="val_macro_f1")
    ax.plot(df["epochs"], df["test_accuracy"], marker="o", label="test_accuracy")
    ax.plot(df["epochs"], df["test_macro_f1"], marker="o", label="test_macro_f1")
    ax.set_xlabel("Количество эпох")
    ax.set_ylabel("Метрика")
    ax.set_title("Влияние количества эпох на качество модели")
...
def main() -> int:
    ...
    epoch_values = [int(v.strip()) for v in args.epoch_values.split(",") if v.strip()]
    results: list[dict[str, float | int]] = []

    for epochs in epoch_values:
        print(f"\n=== epochs={epochs} ===")
        metrics = train_single_run(x_train, y_train, x_val, y_val, x_test, y_test, cfg, epochs)
        metrics["epochs"] = epochs
        metrics["feature_count"] = len(feature_names)
        results.append(metrics)

    results_df = pd.DataFrame(results).sort_values(["epochs"]).reset_index(drop=True)
