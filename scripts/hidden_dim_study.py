...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Исследование влияния hidden_dim на качество модели"
    )
    ...
    parser.add_argument(
        ...
        help="Список значений hidden_dim через запятую",
    )
    parser.add_argument(
        "--out-prefix",
        type=str,
        default="hidden_dim_study",
...
def save_plot(df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.plot(df["hidden_dim"], df["val_accuracy"], marker="o", label="val_accuracy")
    ax.plot(df["hidden_dim"], df["val_macro_f1"], marker="o", label="val_macro_f1")
    ax.plot(df["hidden_dim"], df["test_accuracy"], marker="o", label="test_accuracy")
    ax.plot(df["hidden_dim"], df["test_macro_f1"], marker="o", label="test_macro_f1")
    ax.set_xlabel("Размер скрытого слоя (hidden_dim)")
    ax.set_ylabel("Метрика")
    ax.set_title("Влияние hidden_dim на качество модели")
...
def main() -> int:
    ...
    hidden_dim_values = [int(v.strip()) for v in args.hidden_dim_values.split(",") if v.strip()]
    results: list[dict[str, float | int]] = []

    for hidden_dim in hidden_dim_values:
        print(f"\n=== hidden_dim={hidden_dim} ===")
        metrics = train_single_run(x_train, y_train, x_val, y_val, x_test, y_test, cfg, hidden_dim)
        metrics["hidden_dim"] = hidden_dim
        metrics["feature_count"] = len(feature_names)
        results.append(metrics)

    results_df = pd.DataFrame(results).sort_values(["hidden_dim"]).reset_index(drop=True)
